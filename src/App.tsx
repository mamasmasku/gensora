...
// Konstanta
// ─────────────────────────────────────────────────────────
const FREE_API_KEY_STORAGE_KEY = 'gemini_api_key_scriptmate_free';
...
// ─────────────────────────────────────────────────────────
// Helper: build fetch options dengan auth token
// ─────────────────────────────────────────────────────────
async function buildFetchOptions(body: object, freeApiKey: string, isProUser: boolean) {
  ...
  return {
    ...
    body: JSON.stringify({
      ...body,
      ...(!isProUser && freeApiKey ? { apiKey: freeApiKey } : {}),
    }),
  };
}

// ─────────────────────────────────────────────────────────
// AppHeader Component (exported)
// ─────────────────────────────────────────────────────────
export function AppHeader({
  ...
  onShowBuy: () => void;
  freeApiKey: string;
  onSaveFreeKey: (key: string) => void;
}) {
  const [showKeyInput, setShowKeyInput] = useState(false);
  const [keyInput, setKeyInput] = useState(freeApiKey);

  return (
    <header className="mb-8">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-purple-500">
            ScriptMate & SoraPrompt TikTok GO + Product Affiliate
          </h1>
          <p className="text-lg text-purple-300 mt-1">AI Generator Skrip Narasi & prompt video sora sinematik untuk konten TikTok GO & Produk Affiliate.</p>
        </div>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {/* Badge role */}
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${isAdminUser ? 'bg-yellow-900/50 border-yellow-500 text-yellow-300' : isProUser ? 'bg-purple-900/60 border-purple-500 text-purple-300' : 'bg-gray-800 border-gray-600 text-zinc-400'}`}>
              {isAdminUser ? '⭐ Admin' : isProUser ? '💎 Pro' : '🆓 Free'}
            </span>

            {/* Username */}
            <span className="text-sm text-zinc-300">
              {authProfile?.username || (authProfile as any)?.email?.split('@')[0] || 'User'}
            </span>

            {/* Admin link */}
            {isAdminUser && (
              <button onClick={onGoAdmin} className="text-xs text-yellow-400 hover:text-yellow-300 underline underline-offset-2">
                Admin Panel
              </button>
            )}
            ...
            <button
              onClick={onSignOut}
              className="text-xs text-zinc-500 hover:text-zinc-300 bg-gray-800 border border-gray-700 px-2.5 py-1.5 rounded-lg transition-all"
            >
              Keluar
            </button>
          </div>
          ...
          {isProUser && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-gray-800/80 border border-yellow-700/50 rounded-lg px-3 py-1.5">
                <span className="text-yellow-400">💰</span>
                <span className="text-sm font-bold text-yellow-300">{localCredits}</span>
                <span className="text-xs text-zinc-500">kredit</span>
              </div>
              <button
                onClick={onShowBuy}
                className="text-xs bg-yellow-500 text-gray-900 font-bold px-3 py-1.5 rounded-lg hover:bg-yellow-400 transition-all"
              >
                + Beli Kredit
              </button>
            </div>
          ...
          {/* Free user: API key input */}
          {!isProUser && (
            <div className="flex flex-col items-end gap-1">
              <button
                onClick={() => setShowKeyInput(v => !v)}
                className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${freeApiKey ? 'bg-green-900/30 border-green-700/50 text-green-400' : 'bg-yellow-900/30 border-yellow-700/50 text-yellow-400'}`}
              >
                {freeApiKey ? '🔑 API Key Aktif' : '⚠️ Belum ada API Key'}
              </button>
              {showKeyInput && (
                <div className="flex gap-2 mt-1">
                  <input
                    ...
                    className="text-xs bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-zinc-300 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-purple-500 w-52"
                  />
                  <button
                    onClick={() => { onSaveFreeKey(keyInput.trim()); setShowKeyInput(false); }}
                    className="text-xs bg-purple-600 hover:bg-purple-500 px-3 py-1.5 rounded-lg text-white font-semibold transition-all"
                  >
                    Simpan
                  </button>
                </div>
              ...
              <button
                onClick={onShowBuy}
                className="text-xs bg-gradient-to-r from-yellow-500 to-purple-600 text-white font-bold px-3 py-1.5 rounded-lg hover:opacity-90 transition-all"
              >
                ✨ Upgrade ke Pro
              </button>
            </div>
...
export default function App({
  ...
  const [freeApiKey, setFreeApiKey] = useState<string>(() => {
    try { return localStorage.getItem(FREE_API_KEY_STORAGE_KEY) || ''; } catch { return ''; }
  });
  const [showBuyModal, setShowBuyModal] = useState(false);
  const [localCredits, setLocalCredits] = useState(authCredits);

  useEffect(() => { setLocalCredits(authCredits); }, [authCredits]);

  const handleSaveFreeApiKey = (key: string) => {
    setFreeApiKey(key);
    try {
      if (key) localStorage.setItem(FREE_API_KEY_STORAGE_KEY, key);
      else localStorage.removeItem(FREE_API_KEY_STORAGE_KEY);
    } catch {}
  };

  // ── canGenerate: pro pakai server key, free butuh apiKey sendiri ──
  const canGenerate = isProUser || freeApiKey.trim().length > 0;

  // ── isModeLocked: free user hanya bisa mode bebas ───────
  const isModeLocked = (modeId: string) => !isProUser && modeId !== 'bebas';
  ...
  // ── Jika mode terkunci dan dipilih, arahkan ke upgrade ──
  const handleModeChange = (modeId: string) => {
    if (isModeLocked(modeId)) {
      setShowBuyModal(true);
      return;
    }
    setPromptMode(modeId as PromptModeKey);
  };
  ...
  const handleSkripJualanGenerate = async (config: SkripJualanConfig) => {
    ...
    try {
      ...
      const response = await fetch('/api/gemini', fetchOptions);
      const data = await response.json();

      // Cek kredit habis
      if (data.error === 'KREDIT_HABIS') {
        setShowBuyModal(true);
        setSkripJualanOutput('');
        return;
      }

      // Update kredit jika ada
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-zinc-200 font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ── Header dengan info user & kredit ── */}
        <AppHeader
          ...
          onSignOut={onSignOut}
          onGoAdmin={onGoAdmin}
          onShowBuy={() => setShowBuyModal(true)}
          freeApiKey={freeApiKey}
          onSaveFreeKey={handleSaveFreeApiKey}
        />
              ...
              <div className="flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-4 p-6 bg-gray-800/50 border border-purple-700 rounded-xl">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      const locked = isModeLocked(id);
                      return (
                        <button
                          key={id}
                          onClick={() => handleModeChange(id)}
                          className={`py-3 px-2 rounded-lg font-semibold transition-all text-sm leading-tight flex flex-col items-center gap-1 ${promptMode === id ? 'bg-yellow-500 text-gray-900' : locked ? 'bg-gray-700/30 text-zinc-500 cursor-pointer' : 'bg-gray-700/50 text-white hover:bg-gray-700'}`}
                        >
                          <span className="flex items-center gap-1">{label}{locked && <span className="text-xs">🔒</span>}</span>
                  ...
                  {!isProUser && (
                    <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg px-4 py-3 text-xs text-yellow-400">
                      Mode Rapi, Urai Skrip, dan Skrip Jualan hanya untuk user Pro.{' '}
                      <button onClick={() => setShowBuyModal(true)} className="underline font-semibold">Upgrade sekarang →</button>
                    </div>
                  )}
                </div>
                ...
                {!canGenerate && (
                  <p className="text-xs text-center text-yellow-500 -mt-4">
                    🔑 Masukkan API Key Gemini di pojok kanan atas untuk mengaktifkan generate
                  </p>
                )}
              </div>
          ...
          <>
            <div className="flex flex-col gap-8">
              ...
              <button
                ...
                disabled={isLoading || !canGenerate || (promptMode === 'urai' && !scriptInput.trim())}
              ...
              {canGenerate && promptMode === 'urai' && !scriptInput.trim() && !isLoading && (
      ...
      {/* ── Modal Beli Kredit / Upgrade ── */}
      {showBuyModal && (
        <BuyCreditsModal
          onClose={() => setShowBuyModal(false)}
          onSuccess={(newCredits: number) => {
